...
test('시드 사용자 최소 10건 렌더 + role badge / status badge', async ({ page }) => {
    await loginAdmin(page);
    await page.goto('/admin/users', { waitUntil: 'domcontentloaded' });
    const rows = page.locator('.admin-user-row:not(.admin-user-row--head)');
...
test('role filter SYSTEM_ADMIN 클릭 → URL 반영 · 탭 활성 · 필터된 결과', async ({ page }) => {
    ...
    const badges = page.locator('.admin-user-row:not(.admin-user-row--head) .admin-role-badge');
...
test('검색어 입력 → URL q 파라미터 반영 + 검색어 유지', async ({ page }) => {
    ...
    // 결과에 sysadmin 이메일 노출
    await expect(page.locator('.admin-user-row:not(.admin-user-row--head)').first()).toContainText('sysadmin');
});
...
test('사용자 행 클릭 → 상세 페이지 이동', async ({ page }) => {
    await loginAdmin(page);
    await page.goto('/admin/users', { waitUntil: 'domcontentloaded' });
    // A8 이후 row 는 <div> + 내부 개별 <a class="admin-user-row-link"> 로 분리됨. 첫 링크 클릭.
    const firstRow = page.locator('.admin-user-row:not(.admin-user-row--head)').first();
    await firstRow.locator('a.admin-user-row-link').first().click();
