/*
 * Youth-Moa — 사용자 웹 인터랙티브 프로토타입 (개발 참조용)
 * 원본: Youth-Moa Prototype.html — 동기화된 최신본
 *
 * 주의:
 *  - 디자인 시연용 소스. 실제 구현 시 HANDOFF.md의 토큰/컴포넌트 명세를 따르세요.
 *  - 데이터(PROGRAMS/NOTICES/CENTER_DATA)는 하드코딩 → 실제는 API 연동.
 *  - 반복된 인라인 스타일은 개발 시 Tailwind/CSS 모듈/토큰으로 이전 권장.
 *  - 이 파일은 styled-jsx 없이 React + TS만으로 동작하도록 작성됨.
 *    .screen-enter / .btn-hover 등 애니메이션 클래스는 globals.css에 동일하게 정의 필요.
 */
...
function capInfo(pg){
	...
	if(upcoming){ color=T.secondary; label='진행예정'; }
	...
	return { applied, ratio, pct, full, upcoming, color, label };
}
...
function NotifPanel({ onClose }) {
	const [items, setItems] = useState([
		{ id:1, title:'프로그램 신청 승인', body:'[취업역량 강화 워크숍] 신청이 승인되었습니다.', time:'1시간 전', unread:true },
		{ id:2, title:'마감 임박', body:'[청년 창업 아카데미] 마감이 임박했어요. (D-1)', time:'3시간 전', unread:true },
		{ id:3, title:'공지사항', body:'새 공지사항 — 7월 휴관 일정 안내', time:'1일 전', unread:false },
		{ id:4, title:'신청 취소 처리', body:'[마음건강 힐링 캠프] 취소가 처리되었습니다.', time:'2일 전', unread:false },
	]);
...
function UserMenuPanel({ onClose, onLogout, onMenu }) {
	return (
		<div className="dropdown-enter" style={{ width:220, background:T.surface, borderRadius:14, boxShadow:'0 12px 40px rgba(0,0,0,0.14)', border:`1px solid ${T.borderLight}`, overflow:'hidden' }}>
			...
			{[{icon:'calendar',label:'프로그램 신청 내역',tab:'history'},{icon:'star',label:'즐겨찾기한 프로그램',tab:'favorites'},{icon:'bell',label:'알림 설정',tab:'noti'},{icon:'user',label:'개인 정보 수정',tab:'profile'}].map(m=>(
				<div key={m.label} onClick={()=>{onClose();onMenu(m.tab);}} className="btn-hover" style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 18px', cursor:'pointer' }}>
		...
		<div className="screen-fade" style={{ background:T.bg }}>
			...
			<div style={{ padding:'40px 80px 48px', background:T.primaryBg }}>
				...
				<div style={{ display:'flex', gap:24 }}>
					<div className="card-hover" onClick={()=>go('notices')} style={{ width:360, borderRadius:T.radius, overflow:'hidden', background:T.surface, boxShadow:T.shadowMd, flexShrink:0 }}>
					...
					<div style={{ flex:1, background:T.surface, borderRadius:T.radius, boxShadow:T.shadow, overflow:'hidden' }}>
						{NOTICES.slice(1).map((n,i)=>(
							<div key={n.id} onClick={()=>go('notices')} className="card-hover" style={{ display:'flex', alignItems:'center', gap:14, padding:'0 24px', height:64, borderBottom:i<2?`1px solid ${T.borderLight}`:'none', cursor:'pointer' }}>
...
function ProgramList({ go }) {
	...
	let filtered = PROGRAMS.filter(p=>active==='전체'||p.status===active);
	...
	return (
		<div className="screen-enter" style={{ background:T.bg, minHeight:'100vh' }}>
			...
			<div style={{ display:'flex', padding:'28px 80px 48px', gap:28, alignItems:'flex-start' }}>
				{/* Sidebar — 목록 뷰에서만 */}
				{view==='grid' && <div style={{ width:210, flexShrink:0 }}>
					<div style={{ fontWeight:600, fontSize:14, color:T.text, marginBottom:14 }}>지역</div>
					{regions.map(r=>(
						<label key={r} onClick={()=>setSel(s=>{const n=new Set(s);n.has(r)?n.delete(r):n.add(r);return n;})} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10, cursor:'pointer' }}>
							<div style={{ width:16, height:16, borderRadius:4, border:`1.5px solid ${sel.has(r)?T.primary:T.border}`, background:sel.has(r)?T.primary:T.surface, display:'flex', alignItems:'center', justifyContent:'center', transition:'all 150ms' }}>
								{sel.has(r) && <svg viewBox="0 0 24 24" style={{width:10,height:10}}><path d="M5 12l5 5L20 7" stroke="#fff" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>}
							</div>
		...
		<div className="screen-enter" style={{ background:T.bg, minHeight:'100vh' }}>
			<div style={{ maxWidth:1000, margin:'0 auto', padding:'36px 0 48px' }}>
				...
				{inactive && (
					<div style={{ display:'flex', gap:12, padding:'16px 20px', borderRadius:T.radius, background:T.warningLight, border:`1px solid ${T.warning}33`, marginBottom:24 }}>
						...
						<div>
							...
							<div style={{ fontSize:13.5, color:T.textSec, lineHeight:1.6 }}>운영 사정으로 해당 프로그램의 신청이 중단되었어요. 이미 신청하셨다면 <strong style={{color:T.text}}>마이페이지 › 신청 내역</strong>에서 확인하실 수 있습니다. 문의는 운영 센터로 연락해주세요.</div>
						</div>
					</div>
				)}
				<div style={{ display:'flex', gap:32 }}>
					...
					<div style={{ flex:1 }}>
						...
						<div style={{ padding:14, borderRadius:T.radius, background:c.full?T.borderLight:T.primaryBg, border:`1px solid ${c.full?T.border:T.primaryLight}`, marginBottom:20 }}>
							<div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
								<span style={{ fontSize:14, fontWeight:700, color:c.color }}>{c.upcoming?`신청 오픈까지 ${pg.openDday}일`:c.full?'모집 마감':c.label}{!c.full && !c.upcoming && ` · 마감까지 ${pg.dday===0?'오늘':pg.dday+'일'}`}</span>
							...
							<CapacityBar pg={pg} showLabel={false}/>
							<div style={{ fontSize:12, color:T.textSec, marginTop:8 }}>{c.upcoming?`${pg.apply? pg.apply.replace('~',' ~ ').replace(/2024-/g,'') : ''} 신청 예정입니다. 오픈 알림을 신청하면 시작 시 알려드려요.`:c.full?'정원이 마감되었습니다. 알림을 신청하면 빈자리가 생길 시 알려드려요.':`현재 신청률 ${c.pct}% · 경쟁률 ${(c.applied/pg.cap).toFixed(1)}:1`}</div>
						</div>
						<div style={{ borderTop:`1px solid ${T.border}` }}>
							{[['신청 기간',pg.apply? pg.apply.replace('~',' ~ ') : '2024-07-01 ~ 07-31'],['진행 기간',pg.date],['진행 장소','__MAP__'],['모집인원',pg.cap+'명'],['문의처','031-123-4567']].map(([k,v])=>(
								<div key={k} style={{ display:'flex', alignItems:'center', padding:'11px 0', borderBottom:`1px solid ${T.borderLight}` }}>
									<span style={{ width:90, fontSize:13, fontWeight:500, color:T.textSec, flexShrink:0 }}>{k}</span>
									{v==='__MAP__' ? (
										<div style={{ display:'flex', alignItems:'center', gap:10 }}>
											<span style={{ fontSize:13, color:T.text }}>{pg.center} (경기도 {pg.region})</span>
											<div className="btn-hover" onClick={()=>setShowMap(true)} style={{ display:'inline-flex', alignItems:'center', gap:4, padding:'4px 10px', borderRadius:T.tagR, border:`1px solid ${T.primary}`, cursor:'pointer' }}>
												<Icon n="pin" size={13} color={T.primary}/><span style={{ fontSize:12, fontWeight:600, color:T.primary }}>지도</span>
											</div>
										</div>
			...
			{showMap && (
				<Modal onClose={()=>setShowMap(false)}>
					<div style={{ width:520, background:T.surface, borderRadius:16, boxShadow:'0 20px 60px rgba(0,0,0,0.18)', overflow:'hidden' }}>
						<div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 22px', borderBottom:`1px solid ${T.borderLight}` }}>
							<span style={{ fontSize:16, fontWeight:700, color:T.text }}>진행 장소</span>
							<div className="btn-hover" onClick={()=>setShowMap(false)} style={{cursor:'pointer'}}><Icon n="close" size={20} color={T.textTri}/></div>
						</div>
						{/* 지도 플레이스홀더 */}
		...
		<div className="screen-enter" style={{ background:T.bg, minHeight:'100vh' }}>
			<div style={{ maxWidth:700, margin:'0 auto', padding:'36px 0 48px' }}>
				...
				<div style={{ display:'flex', flexDirection:'column', gap:14, marginBottom:24 }}>
					{[{label:'이름',val:'박시현',disabled:true},{label:'핸드폰 번호',val:'010-1234-5678',disabled:true},{label:'이메일',val:'hyuuun0321@naver.com',disabled:true,note:'이메일을 통해 프로그램 관련 알림을 드립니다.'}].map(f=>(
						<div key={f.label}>
...
function MyPage({ go, addToast, initialTab }) {
	...
	const appBadge = { '승인':{bg:T.primary,c:'#fff'}, '대기':{bg:T.text,c:'#fff'}, '반려':{bg:T.error,c:'#fff'}, '취소':{bg:'#9CA3AF',c:'#fff'} };
	const doCancel = () => {
		...
		<div className="screen-enter" style={{ background:T.bg, minHeight:'100vh', display:'flex', flexDirection:'column' }}>
			<div style={{ display:'flex', gap:40, padding:'36px 80px', flex:1, alignItems:'flex-start' }}>
				...
				<div style={{ width:220, flexShrink:0 }}>
					...
					{[{key:'history',icon:'calendar',label:'프로그램 신청 현황'},{key:'favorites',icon:'star',label:'즐겨찾기한 프로그램'},{key:'noti',icon:'bell',label:'알림 설정'},{key:'profile',icon:'user',label:'개인 정보 수정'}].map(m=>(
						<div key={m.key} onClick={()=>setTab(m.key)} className="btn-hover" style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', borderRadius:8, background:tab===m.key?T.primaryLight:'transparent', cursor:'pointer', marginBottom:2 }}>
				...
				<div style={{ flex:1 }}>
					{tab==='history' && (
						<div>
							...
							<div style={{ display:'flex', gap:8, marginBottom:20 }}>
								{['3개월','6개월','1년','3년'].map(p=>(
									<div key={p} className="btn-hover" onClick={()=>setPeriod(p)} style={{ flex:1, height:44, borderRadius:8, border:`1px solid ${period===p?T.text:T.border}`, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:14, fontWeight:period===p?700:400, color:period===p?T.text:T.textSec }}>{p}</div>
								))}
							</div>
											...
											<div key={app.id} style={{ padding:'18px 20px', borderRadius:T.radius, background:T.surface, border:`1px solid ${T.borderLight}`, boxShadow:T.shadow }}>
												...
												<div style={{ display:'flex', gap:16 }}>
													...
													<div style={{ flex:1, minWidth:0 }}>
														...
														{/* 액션 버튼 */}
														{canCancel && (
															<div className="btn-hover" onClick={()=>setShowCancel(true)} style={{ height:40, borderRadius:8, border:`1px solid ${T.error}`, display:'flex', alignItems:'center', justifyContent:'center', gap:6, cursor:'pointer' }}>
																<Icon n="close" size={15} color={T.error}/><span style={{ fontSize:13.5, fontWeight:600, color:T.error }}>신청 취소</span>
															</div>
														)}
														{canReapply && (
															<div style={{ display:'flex', gap:8 }}>
																<div className="btn-hover" onClick={()=>go('program-detail',{pg:app})} style={{ flex:1, height:40, borderRadius:8, border:`1px solid ${T.text}`, display:'flex', alignItems:'center', justifyContent:'center', gap:6, cursor:'pointer' }}>
																	<Icon n="check" size={15} color={T.text}/><span style={{ fontSize:13.5, fontWeight:600, color:T.text }}>재신청</span>
																</div>
																<div style={{ flex:1, height:40, borderRadius:8, background:T.borderLight, display:'flex', alignItems:'center', justifyContent:'center', gap:6, cursor:'not-allowed' }}>
																	<Icon n="close" size={15} color={T.textTri}/><span style={{ fontSize:13.5, fontWeight:600, color:T.textTri }}>신청 취소</span>
																</div>
															</div>
														)}
													</div>
					...
					{tab==='profile' && (
						<div>
							...
							<div style={{ fontSize:14, fontWeight:600, color:T.text, marginBottom:8 }}>비밀번호 재확인</div>
					...
					{tab==='noti' && (
						<div>
							<h3 style={{ fontSize:20, fontWeight:700, color:T.text, marginBottom:6 }}>알림 설정</h3>
							<div style={{ height:1, background:T.text, marginBottom:24 }}/>
			...
			{/* Cancel Modal */}
			{showCancel && (
				<Modal onClose={()=>{setShowCancel(false);setCancelError(false);setCancelReason('');}}>
					<div style={{ width:460, padding:'32px 28px 24px', background:T.surface, borderRadius:16, boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }}>
						<h3 style={{ fontSize:19, fontWeight:700, color:T.text, marginBottom:6, textAlign:'center' }}>신청 취소하시겠습니까?</h3>
						<p style={{ fontSize:13, color:T.textSec, marginBottom:20, textAlign:'center', lineHeight:1.6 }}>취소 후에는 다시 신청하셔야 합니다.</p>
						...
						{cancelError && <div style={{ fontSize:12, color:T.error, marginBottom:10 }}>필수 선택 항목입니다.</div>}
						<div style={{ display:'flex', gap:10, marginTop:20 }}>
							<Btn size="m" variant="ghost" fullWidth onClick={()=>{setShowCancel(false);setCancelError(false);setCancelReason('');}}>돌아가기</Btn>
							<Btn size="m" variant="danger" fullWidth onClick={doCancel}>신청 취소</Btn>
						</div>
					</div>
				</Modal>
			)}
		...
		<div className="screen-enter" style={{ background:T.surface, minHeight:'100vh', display:'flex', flexDirection:'column' }}>
			...
			<div style={{ padding:'0 80px 48px', flex:1 }}>
				<div style={{ borderTop:`1px solid ${T.text}`, borderBottom:`1px solid ${T.border}` }}>
					...
					{NOTICES.map((n,i)=>(
						<div key={n.id} className="card-hover" style={{ display:'grid', gridTemplateColumns:'80px 80px 1fr 120px 80px', padding:'14px 16px', borderBottom:`1px solid ${T.borderLight}`, alignItems:'center', background:n.pin?T.primaryBg:'transparent', cursor:'pointer' }}>
						...
						{filtered.length===0
							...
							: filtered.map(c => (
								<div key={c.id} onClick={()=>handleCard(c.id)} className="card-hover"
									 style={{ padding:detailId?'10px 12px':'14px', borderRadius:T.radius, background:c.id===detailId?T.primaryBg:T.surface, border:`1.5px solid ${c.id===detailId?T.primary:T.borderLight}`, cursor:'pointer', transition:'all 200ms' }}>
									{detailId ? (
										<div style={{ display:'flex', alignItems:'center', gap:8 }}>
...
function App() {
	...
	// Flow indicator
	const FLOW = { home:'홈', programs:'프로그램 목록', 'program-detail':'프로그램 상세', 'program-apply':'프로그램 신청', 'apply-complete':'신청 완료', mypage:'마이페이지', 'application-detail':'신청 상세', notices:'공지사항', centers:'청년센터 찾기', search:'검색', login:'로그인', welcome:'회원가입 환영', forbidden:'403 접근 권한 없음', 'error-404':'404 페이지 없음', 'error-503':'503 점검 중', 'session-expired':'세션 만료' };
