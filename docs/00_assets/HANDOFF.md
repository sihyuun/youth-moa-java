...
│   ├─ 이름
│   ├─ 핸드폰 번호 + 인증요청 버튼
│   │   · 인증요청 클릭 → 아래 인증번호 행 노출: 6자리 숫자 input(우측 내부 3:00 카운트다운, 30초 이하 빨강) + [확인]
│   │   · 유효시간 3분 만료 시 안내 문구 + 확인 비활성 / 버튼 상태 흐름: 인증요청 → 재요청(타이머 리셋) → (완료 후) 재인증(상태 초기화+잠금 해제)
│   │   · 인증 완료 → ✓ "휴대폰 인증이 완료되었어요" + 번호 input 잠금
│   ├─ 성별 (라디오)
│   ├─ 생년월일 (캘린더 피커)
│   └─ 주소 (우편번호 검색 + 주소 + 상세주소)
...
│   ├─ 관심 정보: [아이콘+라벨(회색)] + 값칩(primaryLight) 그룹형 — 관심 지역 / 관심 분야
│   │   · 그룹당 값칩 최대 3개 노출 + `+N` 축약칩(muted, title로 나머지 전체) — 다중 선택 오버플로우 대응
│   │   · 값 없으면 "미설정", 끝에 [관심 정보 수정 ✎] 링크 → **관심 정보 수정 모달**(InterestEditModal)
│   │   · 모달: 웰컴 칩 UI 재사용(지역 10개+더보기 텍스트링크, 분야 7종, 선택 개수 표시) + [취소/저장] — 저장 시 카드 즉시 갱신 + 토스트
...
500 — 경고 삼각형(빨강) + "일시적인 오류가 발생했어요" + 안내(잠시 후 재시도 + 문의 유도) + [홈으로(ghost)][다시 시도(primary·refresh)] + Error code 500·문의 — 헤더 숨김
...
### 5.21 온보딩 환영 (`/welcome`) — 가입 직후 1회
```
체크 아이콘(72px primary 원) + "환영합니다, {이름}님! 🎉"
├─ 관심 지역 (칩 다중 선택, 점진 노출)
│   ├─ 제목 옆 선택 카운트: 선택 시 [N개 선택](primary), 미선택 시 "중복 선택 가능"(textTri)
│   ├─ 기본 노출: 인구·이용 상위 10개 시 (수원·성남·고양·용인·부천·안양·안산·화성·남양주·평택)
│   ├─ [지역 더 보기 (N) ▾] **primary 텍스트 링크**(칩 행 아래, 테두리·배경 없음) → 클릭 시 나머지 20개 펼침 / [지역 접기 ▴]
│   └─ 접힌 상태에서도 더보기 영역에서 선택한 지역 칩은 항상 노출 유지 (더보기 카운트에서 차감)
├─ 관심 분야 (칩 다중 선택, 7종) — 제목 옆 동일 선택 카운트
├─ CTA (size l, fullWidth): 선택 시 [관심 정보 N개로 시작하기] / 미선택 시 [관심 정보 저장하고 시작하기]
└─ [나중에 할게요 · 건너뛰기] 텍스트 링크 → 홈
```
> 지역 30개 전체 나열 대신 점진 노출(당근마켓·에어비앤비 온보딩 패턴). 저장 시 토스트 + 홈 이동, 관심 정보는 마이페이지 개인정보 수정에서 변경 가능.
...
· 목록 필터는 지역 + 청년센터 + 상태 탭(전체/모집중/진행예정/종료 — 마감은 필터 아님, 카드 뱃지로만)
· 종료 프로그램: 전체 탭에서 숨김, 「종료」 탭에서만 조회 · 정렬 항상 뒤로
  - 카드: 그레이스케일 썸네일 + 「종료」 회색 뱃지 + "지난 프로그램" 비활성 표시(카드 클릭은 상세 이동 허용)
  - 상세: 회색 패널 "종료된 프로그램" + CTA 「비슷한 프로그램 보기」(목록 이동, dead-end 방지) — sticky 바 동일
  - 상태 구분: 종료(기간 종료, 자연) vs 중단(관리자 조치) — 별개 상태 유지
· status 값: '모집중'|'진행예정'|'종료'|'중단' — 정원 마감은 status가 아니라 applied>=cap 파생 판정
...
### 5-E.5 알림 신청 모달 (`WaitlistModal` — 빈자리/오픈 공용)
```
· 마감 프로그램: 신청버튼 → "빈자리 알림 받기" / 진행예정: "오픈 알림 받기" (모두 outline)
· 두 액션 모두 동일 모달 사용 — mode prop('waitlist'|'open')으로 문구만 분기
  - waitlist: "빈자리가 생기면 아래의 수단으로 알림을 보내드려요."
  - open: "신청이 시작되면 아래의 수단으로 알림을 보내드려요."
· 모달: 대상 프로그램 + 알림 채널 선택(체크박스, 복수: 알림톡/이메일)
· 신청 완료 → 버튼 "알림 신청됨"(카드는 "알림 신청됨 · 해제")으로 전환, 재클릭 시 즉시 해제 + 토스트
...
1. **즐겨찾기 전역 스토어** — 프로토타입은 홈/목록/상세가 각자 로컬 state로 즐겨찾기를 들고 있음(데모 한계). 구현 시 단일 전역 스토어(서버 동기화) 1곳으로 관리하고 마이페이지 KPI·카드 별표와 연동할 것.
2. **비로그인 가드** — 로그인 필요 액션: 신청하기 · 즐겨찾기 · 빈자리 알림 · 오픈 알림. 모두 로그인 유도 모달(LoginPromptModal)로 가드 (프로토타입 반영됨).
3. **데모 전용 요소 — 개발 산출물에서 제외** — 우하단 "현재 화면" 플로팅 인디케이터, 푸터 「관리자」 링크(403 데모 진입용), 로그인 초기값 true.
4. **관심 정보 기반 추천** — 웰컴/마이페이지에서 수집한 관심 지역·분야를 홈 「맞춤 추천」 정렬에 반영 (관심 지역 일치 > 관심 분야 일치 > 마감 임박순).
5. **알림 전체 페이지** — 헤더 패널 하단 「알림 전체보기」 → `/notifications` (패널과 동일 데이터 소스 공유, 읽음 처리 동기화).
6. **목록 0건 빈 상태** — 필터 조합 결과 0건 시 안내 카드 + 「필터 초기화」 버튼(상태 탭·지역/센터·정렬 리셋).
7. **작업용 파일** — `docs/archive/`의 Color Tuner·Hero 배경 시안은 확정 전 탐색 기록용. 개발 범위 아님.
...
| 날짜 | 내용 |
|------|------|
| 2026-07-20 | **종료 상태 신설**(목록 그레이스케일 카드·종료 탭·상세 「비슷한 프로그램 보기」) · 상태 탭 정리(마감 필터 제거, 진행중→모집중) · 알림 모달 공용화(mode: waitlist/open) · 알림 신청됨 토글(재클릭 해제) · 회원가입 카드형 재디자인+공통 inputStyle/inputRO 전 화면 통일 · 개인정보 수정 실제 폼 연결(비밀번호 검증→폼, 탈퇴 확인 다이얼로그) · 알림 설정 즉시 저장 · 알림 전체 페이지(날짜 그룹+안읽음 필터+뒤로가기) |
| 2026-07-16 | 전체 검토 반영: 비로그인 가드(즐겨찾기·빈자리/오픈 알림) · 상태 뱃지 매핑 단일화(statusStyle) · 알림 전체 페이지(/notifications) · 목록 0건 빈 상태+필터 초기화 · 데모 데이터 연도 2026 통일 · 작업파일 docs/archive/ 이동 · §12 개발 이관 유의사항 추가 |
| 2026-07-16 | 관심 정보 수정 모달(A안, 웰컴 칩 UI 재사용) · 500 일시적 오류 화면 · 회원가입 휴대폰 인증(3분 타이머·재요청·재인증) · 전역 input 라이트모드 고정(`:root{color-scheme:light}` + `input,textarea,select{background:#fff}` — 다크모드 지원 시 토큰으로 교체) |
...
| — | 온보딩 지역 선택 점진 노출(§5.21): 상위 10개 시 기본 + 더보기(20개) 토글, 경기도 30개 시·군 전체 지원 |
| — | 온보딩 개선(§5.21): 더보기 점선칩→primary 텍스트 링크, 관심 지역/분야 선택 개수 표시, CTA 동적 문구(N개로 시작하기) |
| — | 마이페이지 프로필 관심태그 개선: 라벨+값칩 분리형, 값칩 3개+`+N` 축약(오버플로우 대응), 관심정보 수정 링크(edit 아이콘 신규) |
...
| — | **홈 히어로 배경 로테이션**(§5.1.1): Method A scrim(브랜드 틴트+하단 darken 2겹) 위에 텍스트 없는 사진 6종 8초 크로스페이드 순환. 후보 8종 시안 `Hero
| — | **공통화 표준 결정 §4-S**: Toast 전역 단일화(alert() 14곳 대체) · Modal 범용 3계층 + M1 포커스트랩/M2 모바일 풀스크린(768px)/M3 다중모달 백드롭 1겹 · Dropdown D1 클릭토글(hover 폐지)/D2 유저메뉴 A안 2항목(§4.13 정정)/D3 radius14·shadow 0 12px 40px rgba(0,0,0,.14). 스크린샷 `screenshots/d3-user-dropdown.png` | 배경 시안.html` 커밋, 운영 시 실제 청년센터 사진 교체 권장 |
...
- **버튼 카피 통일**: 마감 = `빈자리 알림 받기`, 진행예정 = `오픈 알림 받기`, 종료 = `비슷한 프로그램 보기`, 일반 = `신청하기` — 알림 신청 후 `알림 신청됨`(재클릭 = 해제)
