...
|------|------|------|
| `title` | `string` | 모달 제목 |
| `message` | `string` | 본문 설명 |
| `variant` | `'danger' \| 'warning' \| 'info'` | 아이콘/버튼 색상 |
| `confirmText` | `string` | 확인 버튼 라벨 |
| `cancelText` | `string` | 취소 버튼 라벨 |

- 너비: 420px, padding 32px 28px 24px, border-radius 16px
- 그림자: `0 20px 60px rgba(0,0,0,0.15)`
- 배경 dim: `rgba(0,0,0,0.4)`
- 아이콘: 56px 원형 (danger: 빨강 #FEE2E2, warning: 노랑 #FEF3C7, info: primaryLight)
- 버튼: M 사이즈, 좌측=ghost / 우측=danger 또는 primary
- 애니메이션: fadeIn 200ms + scale 0.95→1
- 사용 예: 프로그램 신청 취소, 회원 탈퇴, 로그아웃, 신청 완료

### 4.10 CancelModal (`<YMCancelModal>`) — 신청 취소 사유 선택
...
- 상단: 아바타(이니셜) + {이름}님 + 이메일
- 메뉴: 프로그램 신청 내역 / 즐겨찾기한 프로그램 / 개인 정보 수정
- 하단(구분): 로그아웃
...
```
Header
├─ Hero Banner (420px, 배경 이미지 + gradient overlay + 검색바)
...
Footer
```
> 토스트로 사라지게 하지 않고 **완료 페이지**로 다음 행동(현황 확인)을 안내. 가볍운 피드백(즐겨찾기/복사/취소)만 토스트 사용.
...
| — | 브랜드 로고 적용 (헤더·푸터), 신청하기 버튼 primary 보더 || — | 공지사항 상세 이미지 첨부형, 홈 공지 리스트형 재구성 |
...
| — | 디자인 토큰 → `src/app/globals.css` Tailwind v4 `@theme` 동기화, Utility Class 추가 |
| — | 알림 패널 interactive (삭제/모두읽음/빈상태), 토고 스크롤 감지 헤더 |
| — | 프로토타입 보완: 로고 심벌+텍스트 콤보, 실제 SNS 아이콘 이미지, 푸터 리디자인 |
