...
**정렬 3종** (L862). 어떤 정렬이든 **종료 항목은 항상 뒤로** 밀리는 1차 규칙이 먼저 적용된다 (L827).

| 값 | 라벨 | 규칙 |
|---|---|---|
| `기본` | 기본순 | 정렬 없음 (원본 순서 유지) |
| `마감임박` | 마감임박순 | 정원 마감 항목을 뒤로 → `dday` 오름차순 |
| `인기` | 인기순 | 신청률(`applied/cap`) 내림차순 |
...
## 9. 현재 갭 (2026-07-28 계약 검사 결과)

`e2e/gap-reports/gap-programs.md` 참조. **58/75 통과 · 갭 17건** (P0 0 / P1 6 / P2 11) · 의도적 이탈 2건.

무거운 순서:

1. **P1 — "전체" 탭에 종료 프로그램이 섞여 나온다** (계약 0건, 실제 2건). prototype 은 종료를 전용 탭으로 분리한다. 목록 상단에 지난 프로그램이 올라오는 체감 문제
2. **P1 — 캘린더 뷰 전체 미구현.** 토글이 `disabled` 라 뷰 축이 사실상 1개다. 아이콘(grid/calendar SVG)도 없어 텍스트 버튼 2개로 보인다 (`viewtoggle.enabled`, `viewtoggle.icon`)
3. **P1 — 결과 카운트가 필터 바 밖으로 내려갔다.** prototype 은 우측 그룹 안 정렬 왼쪽. 구현은 필터 바 아래 별도 줄(`.active-filter-bar`)
4. **P1 — 첫 정렬 옵션이 `최신순`** (계약 `기본순`). 라벨뿐 아니라 의미가 다르다 — §10 Q1
5. **P1 — 카드 CTA 가 radius 8 사각 버튼** (계약 20 pill). 카드 안에서 가장 눈에 띄는 형태 차이
6. **P2 묶음 — 카드 내부 리듬이 전반적으로 헐겁다**: 본문 상단 패딩 14(계약 12), 제목 하단 6(3), 센터명 하단 4(2), 기간 하단 8(10). 합쳐서 카드가 prototype 보다 세로로 길어진다
7. **P2 — 뷰 토글 패딩 12/폰트 12** (계약 14/12.5), **페이지네이션 상단 여백 36** (28), **별 색 `0.9` 알파** (0.85), **CTA 아이콘 14** (15), **정렬 gap 4** (2)
...
| # | 항목 | prototype | 현재 | 논점 |
|---|---|---|---|---|
| Q1 | 첫 정렬 옵션 | `기본순` (정렬 미적용) | `최신순` (등록일 내림차순) | 실데이터에서는 "기본순"이 의미를 갖기 어려워 최신순으로 바꾼 것일 수 있음. 라벨만 맞출지, 정렬 규칙까지 맞출지 결정 필요 (`sort.first.text`) |
| Q2 | 지역 · 청년센터 필터 결합 | 하나의 Set, **OR** 결합 | 별도 파라미터, 교차 결합 추정 | prototype 쪽이 데모 편의를 위한 단순화일 가능성이 큼. AND 유지가 맞다면 계약이 아니라 이 문서에 확정 기록 |
| Q3 | 드롭다운 적용 방식 | 체크 즉시 반영, 푸터 없음 | 헤더(제목·×) + 취소/적용 푸터 | 다중 선택 시 요청 수를 줄이려는 의도적 선택일 수 있음 (`popover.noApplyFooter`) |
...
Q1·Q3 를 "현행 유지" 로 결정하시면 검사 대상 갭은 15건이 됩니다.
