...
`e2e/gap-reports/gap-home.md` 참조. **22/43 통과 · 갭 21건** (P0 2 / P1 11 / P2 8).
...
| # | 항목 | prototype | 현재 | 논점 |
|---|---|---|---|---|
| Q1 | Hero 타이틀 카피 | 경기도 청년의 내일을 / 함께 만들어갑니다 | 청년의 모든 기회를 / 한곳에서 | 서비스명이 `청년모아` 로 정착했으므로 현재 카피가 의도적 선택일 수 있음 |
| Q2 | Hero 설명 카피 | 경기도 31개 시·군 청년센터의 프로그램을 한눈에 확인하세요 | 프로그램·공간·정책을 청년모아에서 만나보세요 | Q1 과 같은 건 |
| Q3 | Stats 숫자 | 127 / 31 / 15,420 (목업) | 실제 시드값 | 실데이터 연동이 맞으므로 **계약에서 제외 권장** (이미 미포함) |
| Q4 | 카드 이미지 높이 | 170(비로그인) / 150(로그인) | 둘 다 160 | 한 값으로 통일한 게 의도인지, 단순 누락인지 |

Q1·Q2 를 "현재 카피 유지" 로 결정하시면 계약에서 두 항목을 `deviation` 처리하고, 남는 갭은 19건이 됩니다.
