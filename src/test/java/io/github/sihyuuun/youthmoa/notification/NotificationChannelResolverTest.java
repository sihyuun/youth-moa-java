 ...
 * <p>
 * 채널 활성 조합 4가지 (3/2/1/0) 케이스와 enum 순서 보존을 검증.
 */
class NotificationChannelResolverTest {
    ...
    private User buildUser(boolean kakao, boolean sms, boolean email) {
        return User.builder()
                .email("t@t.t").name("t").role(UserRole.USER)
                .notifyKakao(kakao).notifySms(sms).notifyEmail(email)
                .build();
    }
