 ...
 *
 * DataInitializer 는 !test 프로파일 전용이므로 여기서 시드하지 않는다.
 * 대신 각 테스트에서 필요한 최소 데이터만 준비.
 */
@DataJpaTest
...
class HomeServiceTest {
    ...
    void setUp() {
        // 사이트 이미지 4 slot
        siteImageRepository.saveAll(List.of(
                SiteImage.builder().slot("HERO_BANNER").imageUrl("hero.jpg").sortOrder(0).isActive(true).build(),
                SiteImage.builder().slot("HOME_SPACE_1").imageUrl("s1.jpg").sortOrder(1).isActive(true).build(),
                SiteImage.builder().slot("HOME_SPACE_2").imageUrl("s2.jpg").sortOrder(2).isActive(true).build(),
                SiteImage.builder().slot("HOME_SPACE_3").imageUrl("s3.jpg").sortOrder(3).isActive(true).build()
        ));
        // Notice
        ...
        // Center
        centerRepository.save(Center.builder().name("C1").region("서울").address("A").phone("0").isFeatured(true).build());
        // Program
        LocalDate today = LocalDate.now();
        programRepository.saveAll(List.of(
                Program.builder().title("P1").organization("O").category("취업").region("서울")
                        .content("c").requirements("r").endDate(today.plusDays(1)).isActive(true).build(),
                Program.builder().title("P2").organization("O").category("창업").region("경기")
                        .content("c").requirements("r").endDate(today.plusDays(10)).isActive(true).build(),
                Program.builder().title("P3").organization("O").category("힐링").region("서울")
                        .content("c").requirements("r").endDate(today.plusDays(20)).isActive(true).build(),
                Program.builder().title("P4-inactive").organization("O").category("교육").region("서울")
                        .content("c").requirements("r").endDate(today.plusDays(3)).isActive(false).build()
        ));
    }
    ...
    void recommendedPrograms_interestsMatch() {
        User user = userRepository.save(User.builder()
                .email("u@test.com").password("p").name("테스터")
                .interests(Set.of("힐링"))
