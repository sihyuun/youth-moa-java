...
/**
 * ApplicationRepository.countByProgramIdsAndStatuses 단위 테스트.
 * N+1 방지용 IN 쿼리가 올바른 프로그램별 count 를 반환하는지 검증.
 */
@DataJpaTest
...
class ApplicationRepositoryCountTest {
    ...
    @BeforeEach
    void setUp() {
        programA = programRepository.save(Program.builder()
                .title("프로그램 A").organization("기관")
                .content("내용").requirements("조건")
                .startDate(LocalDate.now().minusDays(1))
                .endDate(LocalDate.now().plusDays(10))
                .capacity(10).build());

        programB = programRepository.save(Program.builder()
                .title("프로그램 B").organization("기관")
                .content("내용").requirements("조건")
                .startDate(LocalDate.now().minusDays(1))
                .endDate(LocalDate.now().plusDays(10))
                .capacity(5).build());

        programC = programRepository.save(Program.builder()
                .title("프로그램 C").organization("기관")
                .content("내용").requirements("조건")
                .startDate(LocalDate.now().minusDays(1))
                .endDate(LocalDate.now().plusDays(10))
                .capacity(20).build());
        ...
        user1 = userRepository.save(User.builder()
                .email("u1@test.com").password("pw").name("유저1")
                .role(UserRole.USER).build());
        user2 = userRepository.save(User.builder()
                .email("u2@test.com").password("pw").name("유저2")
                .role(UserRole.USER).build());
        user3 = userRepository.save(User.builder()
                .email("u3@test.com").password("pw").name("유저3")
                .role(UserRole.USER).build());
    }

    private Application apply(User user, Program program, ApplicationStatus status) {
        return applicationRepository.save(Application.builder()
                .user(user).program(program).status(status).build());
    }
    ...
    void countByProgramIds_correctPerProgram() {
        ...
        List<ApplicationStatus> statuses = List.of(ApplicationStatus.PENDING, ApplicationStatus.APPROVED);
    ...
    void countByProgramIds_emptyIds_returnsEmpty() {
        ...
        List<Object[]> rows = applicationRepository.countByProgramIdsAndStatuses(
