 ...
 *
 * <ul>
 *   <li>정상: 200 + view + model 확인
 *   <li>권한 위반: 다른 유저의 applicationId → 404 (403 X, 존재 노출 방지)
 * </ul>
 */
...
class ApplicationCompleteControllerTest {
  ...
  void success_rendersComplete() throws Exception {
    ...
    given(applicationRepository.findWithProgramAndUserById(123L))
    ...
    mockMvc
        .perform(get("/apply/complete").param("applicationId", "123"))
        .andExpect(status().isOk())
        .andExpect(view().name("application/complete"))
        .andExpect(
            model().attributeExists("myApplication", "program", "channels", "channelSubtitle"))
  ...
  void missingApplication_returns404() throws Exception {
    ...
    mockMvc
        .perform(get("/apply/complete").param("applicationId", "999"))
        .andExpect(status().isNotFound());
  }
