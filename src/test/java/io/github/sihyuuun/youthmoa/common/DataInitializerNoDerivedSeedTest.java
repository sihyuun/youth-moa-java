 ...
 * 규칙·featuredDesc 하드코딩) 도 완전 삭제되고 centers-content.csv 를 진리 소스로 사용해야 한다. DataInitializer.java
 * 소스에서 해당 식별자가 재도입되지 않았는지 문자열 부재로 방어한다.
 *
...
class DataInitializerNoDerivedSeedTest {
  ...
  void 파생_시드_식별자_부재() throws Exception {
    Path source =
        Path.of("src/main/java/io/github/sihyuuun/youthmoa/common/DataInitializer.java");
    ...
    assertThat(content)
        .as("descByKeyword 파생 매핑 로직 재도입 방지")
        .doesNotContain("descByKeyword");
    assertThat(content)
        .as("featuredDesc 하드코딩 로직 재도입 방지")
        .doesNotContain("featuredDesc");
    assertThat(content)
        .as("imgIdx 로테이션 인덱스 재도입 방지")
        .doesNotContain("imgIdx");
  }
}
