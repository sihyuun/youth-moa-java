...
public class UserController {
    ...
    @PostMapping("/signup")
    public String signUp(@Validated(SignUpRequest.OrderedChecks.class)
                         @ModelAttribute SignUpRequest signUpRequest,
                         BindingResult bindingResult,
                         Model model) {
        if (bindingResult.hasErrors()) {
            // password 의 @Size + @Pattern 위반을 한 문장으로 통합 → model 의 passwordPolicyMsg.
            String unified = buildUnifiedPasswordPolicyMessage(signUpRequest.getPassword(),
                                                                bindingResult);
            if (unified != null) {
    ...
    /**
     * 비밀번호 정책 위반을 한 문장으로 조립. JS 의 buildMessage 와 동일 로직.
     * - NotBlank 만 위반 (입력 자체 없음) → null 반환 → 기존 NotBlank 메시지가 그대로 표시
     * - 정책 (@Size / @Pattern) 위반 → "비밀번호는 X 합니다." 또는 "비밀번호는 X 하고, Y 합니다."
     */
