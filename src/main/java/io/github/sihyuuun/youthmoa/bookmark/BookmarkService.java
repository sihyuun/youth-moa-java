...
public class BookmarkService {
     ...
     * 즐겨찾기 토글.
     * <ul>
     *   <li>이미 즐겨찾기 → 삭제 후 {@code false} 반환</li>
     *   <li>없으면 → 생성 후 {@code true} 반환</li>
     * </ul>
     */
    @Transactional
    ...
    public boolean isBookmarked(String userEmail, Long programId) {
        ...
        return userRepository.findByEmail(userEmail)
                .flatMap(u -> programRepository.findById(programId)
                        .map(p -> bookmarkRepository.existsByUserAndProgram(u, p)))
                .orElse(false);
    }

    /**
     * 사용자가 즐겨찾기한 프로그램 ID Set.
     * 카드 N개 그릴 때 단일 쿼리로 가져와 템플릿에서 {@code bookmarkedIds.contains(p.id)} 로 분기.
