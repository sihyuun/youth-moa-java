...
public class ProgramSpec {
    ...
    /**
     * 인기순 정렬 — applied_count / capacity DESC.
     * Specification.toPredicate 안에서 query.orderBy() 로 정렬을 주입하고 predicate 는 conjunction(=true) 반환.
     * 다른 Specification 과 .and() 로 결합 가능.
     */
    public static Specification<Program> orderByPopularity() {
        return (root, query, cb) -> {
            ...
            if (query != null && Long.class != query.getResultType() && long.class != query.getResultType()) {
                ...
                jakarta.persistence.criteria.Root<io.github.sihyuuun.youthmoa.application.Application> appRoot =
                        appSubquery.from(io.github.sihyuuun.youthmoa.application.Application.class);
                appSubquery.select(cb.count(appRoot))
                        .where(
                                cb.equal(appRoot.get("program"), root),
                                appRoot.get("status").in(
                ...
                jakarta.persistence.criteria.Expression<Integer> capExpr = cb.<Integer>selectCase()
