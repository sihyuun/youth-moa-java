...
public class SecurityConfig {
    ...
    /**
     * remember-me 의 PersistentToken 저장소.
     * - persistent_logins 테이블은 PersistentLogin Entity 로 ddl-auto 자동 관리 → createTableOnStartup(false)
     * - 같은 series 의 옛 token 사용 시 Spring 이 자동으로 도난 의심 → 전체 무효화
     */
    @Bean
    ...
    public SecurityFilterChain securityFilterChain(
            ...
            @Value("${security.remember-me.key}") String rememberMeKey) throws Exception {
        http
                .authorizeHttpRequests(auth -> auth
                        // 인증 페이지는 우선 permit (Spring Security 7 매처 동작 이슈 회피용 단독 명시)
                        .requestMatchers("/login", "/signup", "/api/users/check-email").permitAll()
